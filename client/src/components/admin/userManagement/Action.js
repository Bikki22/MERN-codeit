import { FaPencil } from "react-icons/fa6";
import { toast } from "react-toastify";
import { useState } from "react";
import Modal from "@/components/Modal";
import { updateUser } from "@/api/users";
import { FaRegUser } from "react-icons/fa";
import { useRouter } from "next/navigation";

const Action = ({ id, userRoles = [] }) => {
  const [showModal, setShowModal] = useState(false);
  const [roles, setRoles] = useState(userRoles);

  const router = useRouter();

  function updateRoles(role) {
    let updatedRoles = roles;

    if (updatedRoles.includes(role)) {
      updatedRoles = updatedRoles.filter((item) => item != role);
    } else {
      updatedRoles.push(role);
    }
    setRoles(updatedRoles);
  }

  function updateUserRoles() {
    updateUser(id, { roles })
      .then(() => {
        toast.success(`User update success.`, { autoClose: 1500 });
        router.refresh();
      })
      .catch(() => {
        toast.error("User update failed.", { autoClose: 1500 });
      })
      .finally(() => {
        setShowModal(false);
      });
  }

  return (
    <div className="flex items-center gap-4 justify-center">
      <button
        onClick={() => setShowModal(true)}
        className="text-blue-600 cursor-pointer"
      >
        <FaPencil />
      </button>

      <Modal
        icon={<FaRegUser className="mx-auto text-6xl text-gray-400 mb-5" />}
        showModal={showModal}
        setShowModal={setShowModal}
        label={"Update user status"}
        info={
          <div className="pb-5 cursor-pointer flex items-center justify-center gap-4">
            <div className="flex items-center ">
              <input
                type="checkbox"
                id="admin"
                className="mr-1 cursor-pointer"
                defaultChecked={roles.includes("ADMIN")}
                onClick={() => updateRoles("ADMIN")}
              />
              <label htmlFor="admin" className="text-xs">
                ADMIN
              </label>
            </div>
            <div className="flex items-center">
              <input
                type="checkbox"
                id="merchant"
                className="mr-1 cursor-pointer"
                defaultChecked={roles.includes("MERCHANT")}
                onClick={() => updateRoles("MERCHANT")}
              />
              <label htmlFor="merchant" className="text-xs">
                MERCHANT
              </label>
            </div>
            <div className="flex items-center">
              <input
                type="checkbox"
                id="user"
                className="mr-1 cursor-pointer"
                defaultChecked={roles.includes("USER")}
                onClick={() => updateRoles("USER")}
                disabled
              />
              <label htmlFor="user" className="text-xs">
                USER
              </label>
            </div>
          </div>
        }
        confirmAction={
          <button
            onClick={updateUserRoles}
            className="bg-green-700 text-white px-6 py-2 rounded-lg cursor-pointer hover:bg-green-800"
          >
            Update
          </button>
        }
      />
    </div>
  );
};

export default Action;
